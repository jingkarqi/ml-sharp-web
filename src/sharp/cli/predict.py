...
@click.option(
    ...
    default=DEFAULT_CHECKPOINT_PATH if DEFAULT_CHECKPOINT_PATH.exists() else None,
    help=(
        "Path to the .pt checkpoint. Defaults to the local model path if present; otherwise "
        "downloads the default model automatically."
    ),
    required=False,
)
@click.option(
